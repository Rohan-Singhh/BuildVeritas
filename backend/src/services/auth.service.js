...
class AuthService {
    ...
    async _performRegistration(data) {
        ...
        // Step 2: Create user, generate token, and prepare cache in parallel
        const [user, token, cacheKey] = await Promise.all([
            this.userRepository.create(userData),
            this.generateToken(userData),  // We can generate token before user creation since we have all data
